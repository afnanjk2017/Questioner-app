...
public class MyDbHelper extends SQLiteOpenHelper {
    ...
    public int changepass(String username,String password){
        ...
        SQLiteDatabase database = this.getReadableDatabase();
        try (Cursor cursor = database.rawQuery("SELECT * FROM " + TABLE_USERS + " WHERE username = '" + username  + "';", null)) {
            if(cursor.getCount() == 1){
                cursor.close();
                databas.execSQL("UPDATE "+TABLE_USERS+" SET password = "+"'"+password+"' "+ "WHERE username = "+"'"+username+"'"+";");
                return 1;
            }else {
                cursor.close();
                return -1;
    ...
    public ArrayList<Result> getResult(){
        ...
        try (Cursor cursor = database.rawQuery("SELECT * FROM USERDATA ", null)) {
            if(cursor.moveToFirst()){
                do {
                    result.add(new Result(cursor.getString(1),cursor.getString(4),cursor.getString(3)));
                }while (cursor.moveToNext());
            }
        }
        return result;
